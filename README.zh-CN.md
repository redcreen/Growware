...
这个仓库已经从“讨论与文档基线”阶段推进到了“单项目 pilot baseline 已落地”阶段。
...
- 把 `openclaw-task-system` 锁定为 `Project 1`
- 在目标项目内落下项目级 `.growware/` 控制面
- 把 OpenClaw 里的 `feishu6-chat` 安全绑定到专用 `growware` agent
- 跑通本地 deploy baseline：默认安装被宿主拦截时，自动回退到 installed runtime sync、Gateway 重启、smoke 和 drift 校验
- 验证 `growware` agent 可以在 `openclaw-task-system` 工作区内被直接拉起
