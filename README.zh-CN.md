...
这个仓库仍然处在 discussion 和 documentation mode。

当前仓库已经能保存起源材料、定义 policy source，并把第一条 pilot loop 的进入条件写清楚；但它还不能被解读成 `Project 1` 的 runtime 接线已经真实落地。
...
- 把当前 review 主线切回 Growware 自身，并在 [docs/reference/growware/daemon-foundation-plan.zh-CN.md](docs/reference/growware/daemon-foundation-plan.zh-CN.md) 里定义 daemon-first 规划
...
- 用中等规模的 `.codex/*` 控制面把当前阶段对齐到 runtime 前状态
...
7. 最后看阶段顺序：[docs/roadmap.zh-CN.md](docs/roadmap.zh-CN.md)
8. 查看 daemon contract pack：[docs/reference/growware/daemon-contracts/README.zh-CN.md](docs/reference/growware/daemon-contracts/README.zh-CN.md)
9. 查看 policy source：[docs/policy/README.zh-CN.md](docs/policy/README.zh-CN.md) 和 [docs/reference/growware/shared-policy-contract.zh-CN.md](docs/reference/growware/shared-policy-contract.zh-CN.md)
10. 生成并校验机器层：
