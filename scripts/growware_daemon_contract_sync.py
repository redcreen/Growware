...
DAEMON_CONTRACT_SOURCE_DIR = Path("docs") / "reference" / "growware" / "daemon-contracts"
DAEMON_FOUNDATION_DIR = Path(".growware") / "daemon-foundation"
DAEMON_CONTRACT_VERSION = 1


@dataclass(frozen=True)
class DaemonContractDocSpec:
    stem: str
    contract_id: str
    contract_type: str
...
DOC_SPECS: tuple[DaemonContractDocSpec, ...] = (
    DaemonContractDocSpec(
        stem="daemon-boundary",
        contract_id="growware.daemon.boundary",
    ...
    DaemonContractDocSpec(
        stem="project-capsule",
        contract_id="growware.project-capsule.minimum",
    ...
    DaemonContractDocSpec(
        stem="channel-command-model",
        contract_id="growware.channel.command-model.v1",
    ...
    DaemonContractDocSpec(
        stem="progress-push",
        contract_id="growware.progress-push.v1",
    ...
    DaemonContractDocSpec(
        stem="policy-loading",
        contract_id="growware.policy-loading.v1",
    ...
    DaemonContractDocSpec(
        stem="execution-handoff",
        contract_id="growware.execution-handoff.v1",
    ...
    DaemonContractDocSpec(
        stem="learning-writeback",
        contract_id="growware.learning-writeback.v1",
...
def _source_paths(root: Path, spec: DaemonContractDocSpec) -> tuple[Path, Path]:
    en_path = root / DAEMON_CONTRACT_SOURCE_DIR / f"{spec.stem}.md"
    zh_path = root / DAEMON_CONTRACT_SOURCE_DIR / f"{spec.stem}.zh-CN.md"
    return en_path, zh_path


def build_daemon_contract_catalog(project_root: Path | None = None) -> dict[str, Any]:
    root = resolve_project_root(project_root)
    source_dir = root / DAEMON_CONTRACT_SOURCE_DIR
    compiled_dir = root / DAEMON_FOUNDATION_DIR
    ...
    for spec in DOC_SPECS:
        ...
        if not en_path.exists():
            warnings.append(f"missing daemon contract source: {_relative(en_path, root)}")
            continue
        if not zh_path.exists():
            warnings.append(f"missing daemon contract translation: {_relative(zh_path, root)}")
        parsed = _parse_markdown_contract_doc(en_path)
        metadata = parsed["metadata"]

        contract_id = str(metadata.get("id") or spec.contract_id)
        kind = str(metadata.get("kind") or "daemon-contract")
        ...
        contract = {
            "schema": "growware.daemon_foundation.contract.v1",
            "id": contract_id,
    ...
    manifest = {
        "schema": "growware.daemon_foundation.manifest.v1",
        "contract_version": DAEMON_CONTRACT_VERSION,
        "generated_by": "growware/scripts/growware_daemon_contract_sync.py",
        "generated_at": datetime.now(tz=timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
        "index": ".growware/daemon-foundation/index.json",
        "contracts_dir": ".growware/daemon-foundation/contracts",
    }
    index = {
        "schema": "growware.daemon_foundation.index.v1",
        "contracts": [
            {
                "id": contract["id"],
                "contract_type": contract["contract_type"],
                "status": contract["status"],
                "contract_file": f".growware/daemon-foundation/contracts/{source_doc['stem']}.json",
    ...
    provenance = {
        "schema": "growware.daemon_foundation.provenance.v1",
        "contract_version": DAEMON_CONTRACT_VERSION,
    ...
    return {
        ...
        "daemonContractSourceDir": str(source_dir),
        "daemonFoundationDir": str(compiled_dir),
        "contracts": contracts,
        "warnings": warnings,
        "files": files,
    }
...
def write_daemon_contract_catalog(project_root: Path | None = None) -> dict[str, Any]:
    report = build_daemon_contract_catalog(project_root)
    changed = False
...
def validate_daemon_contract_catalog(project_root: Path | None = None) -> dict[str, Any]:
    report = build_daemon_contract_catalog(project_root)
    root = resolve_project_root(project_root)
    missing: list[str] = []
    mismatches: list[str] = []
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compile or validate Growware daemon contract docs into .growware.")
...
def render_markdown(payload: dict[str, Any]) -> str:
    lines = ["# Growware Daemon Contract Sync", ""]
    ...
    lines.append(f"- daemon_contract_source_dir: `{payload['daemonContractSourceDir']}`")
    lines.append(f"- daemon_foundation_dir: `{payload['daemonFoundationDir']}`")
...
def main() -> int:
    ...
    payload = write_daemon_contract_catalog(args.project_root) if args.write else validate_daemon_contract_catalog(args.project_root)
    if args.json:
